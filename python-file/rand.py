...
class home_page(Frame):
    ...
    def __init__(self, parent, controller):
        # image2 = Image.open('backgroundtk.jpg')
        # image1 = ImageTk.PhotoImage(image2)
        # Frame.configure(self, background='backgroundtk.jpg')
        # Frame.configure(background = image1)
        Frame.__init__(self, parent) ### here goes the background##########
        c = Canvas(self,render)
        c.pack(fill=BOTH, anchor='nw', expand=True)
        c.create_text(50, 50, text='Welcome to littleKiddie')
        load = Image.open('backgroundtk.jpg')
        render = ImageTk.PhotoImage(load)
        # img = Label(self, image = render)
        # img.place(x=0,y=0)
        # c.create_image(0, 0, image=render)
